await import('./build-broad-hs300');
await import('./build-broad-cn-core');
await import('./build-broad-cn-szse');
await import('./build-cn-broad');
await import('./build-cn-sectors');
await import('./build-cn-clusters');

await import('./build-us-sp500');
await import('./build-us-nasdaq');
await import('./build-us-dow');
await import('./build-us-semi');
await import('./build-us-xlk');
await import('./build-us-fin');
await import('./build-us-mag7');

await import('./build-manifest');

